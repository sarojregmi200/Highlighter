...
export default function HighlightUpdateForm(props) {
  const {
    id: idProp,
    highlight: highlightModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = highlightRecord
      ? { ...initialValues, ...highlightRecord }
      : initialValues;
    setXpath(cleanValues.xpath);
    ...
    setHtmlMarkup(cleanValues.htmlMarkup);
    setErrors({});
  };
  const [highlightRecord, setHighlightRecord] =
    React.useState(highlightModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Highlight, idProp)
        : highlightModelProp;
      setHighlightRecord(record);
    };
    queryData();
  }, [idProp, highlightModelProp]);
  React.useEffect(resetStateValues, [highlightRecord]);
  const validations = {
    xpath: [{ type: "Required" }],
    data: [{ type: "Required" }],
    color: [{ type: "Required" }],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Highlight.copyOf(highlightRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "HighlightUpdateForm")}
      {...rest}
    >
      <TextField
        label="Xpath"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || highlightModelProp)}
