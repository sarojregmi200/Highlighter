...
export interface Document
  extends Record<string, SearchableValue | Document | unknown> {}
...
export type CustomSorterFunction = (
  a: CustomSorterFunctionItem,
  b: CustomSorterFunctionItem
) => number;
/**
...
export type SearchParams<T = Result[]> = {
   ...
   * attributes or characteristics, such as topic, price, or location.
   ...
   *  properties: ['title', 'description', 'topic.primary', 'topic.secondary'],
   *  facets: {
   *    'topic.primary': {
   *      size: 10,
   *      sort: 'ASC',
...
export type IIndexInsertOrRemoveHookFunction<
  I extends OpaqueIndex = OpaqueIndex,
  R = void
> = (
  index: I,
  prop: string,
...
export interface IIndex<I extends OpaqueIndex = OpaqueIndex> {
  ...
  getSearchablePropertiesWithTypes(
    index: I
  ): SyncOrAsyncValue<Record<string, SearchableType>>;
  load<R = unknown>(
    sharedInternalDocumentStore: InternalDocumentIDStore,
    raw: R
  ): SyncOrAsyncValue<I>;
...
export interface IDocumentsStore<
  D extends OpaqueDocumentStore = OpaqueDocumentStore
> {
  ...
  getMultiple(
    store: D,
    ids: DocumentID[]
  ): SyncOrAsyncValue<(Document | undefined)[]>;
...
export interface ISorter<So extends OpaqueSorter = OpaqueSorter> {
  ...
  getSortablePropertiesWithTypes(
    sorter: So
  ): SyncOrAsyncValue<Record<string, SortType>>;
}
...
export type DefaultTokenizerConfig = {
  ...
  stopWords?:
    | boolean
    | string[]
    | ((stopWords: string[]) => string[] | Promise<string[]>);
...
export interface FunctionComponents<S extends Schema = Schema> {
  validateSchema(
    doc: Document,
    schema: S
  ): SyncOrAsyncValue<string | undefined>;
  getDocumentIndexId(doc: Document): SyncOrAsyncValue<string>;
  getDocumentProperties(
    doc: Document,
    paths: string[]
  ): SyncOrAsyncValue<Record<string, string | number | boolean>>;
  formatElapsedTime(
    number: bigint
  ): SyncOrAsyncValue<number | string | object | ElapsedTime>;
}
...
> = FunctionComponents &
  ArrayCallbackComponents<RequiredInner<P>> &
  Internals<RequiredInner<P>> &
  OramaID;
export {};
